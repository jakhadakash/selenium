...
class TestSuperloop():
    ...
    def test_superloop_flow(self):
        ...
        # ================= STEP 19: Final confirm (Optional) =================
        try:
            ...
            ), timeout=10, desc="final_btn")
            self.driver.execute_script("arguments[0].click();", final_btn)
            time.sleep(3)
            print("✅ Final button clicked", flush=True)
        except TimeoutException:
            print("⚠️ Final button not found - likely already on confirmation page", flush=True)
        ...
        screenshot_path = os.path.join(screenshot_dir, f"final_confirmation_{timestamp}.png")
        
        # Scroll to top to capture the whole page
        self.driver.execute_script("window.scrollTo(0, 0);")
        time.sleep(1)
        
        self.driver.save_screenshot(screenshot_path)
        print(f"✅ Screenshot saved: {screenshot_path}", flush=True)
        print(f"✅ Final URL: {self.driver.current_url}", flush=True)
